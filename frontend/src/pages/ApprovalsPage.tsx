...
export default function ApprovalsPage() {
  ...
  return <div className="max-w-5xl mx-auto space-y-6">
    ...
    <div className="flex items-center gap-1 border-b border-border-subtle pb-2">{(['pending', 'approved', 'rejected', 'all'] as const).map((tab) => <button key={tab} onClick={() => setActiveTab(tab)} className={`px-3 py-1.5 rounded-md text-xs capitalize ${activeTab === tab ? 'bg-bg-subtle text-text-primary' : 'text-text-muted'}`}>{tab} <span className="ml-1 font-mono text-[10px]">{tab === 'all' ? approvals.length : counts[tab]}</span></button>)}</div>
